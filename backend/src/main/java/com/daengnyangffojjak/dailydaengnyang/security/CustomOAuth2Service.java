...
import com.daengnyangffojjak.dailydaengnyang.exception.OAuthException;
...
public class CustomOAuth2Service implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		String registrationId = userRequest.getClientRegistration()
				.getRegistrationId(); //사용한 OAuth 서비스 이름
		//OAuth 서비스에 따라 유저정보를 공통된 class인 UserProfile 객체로 만들어 준다.
		Map<String, Object> customAttributes = OAuthAttribute.getAttribute(registrationId,
				attributes);
		...
		try {
			...
			throw new OAuthException("DUPLICATED_EMAIL");
		}
	...
	private User saveOrUpdate(User user) {
		...
		User updated = userRepository.findByUserName(userName)
				.map(u -> u.update(userName,
						(String) user.getAttributes().get("email"))) //OAuth 서비스 유저정보 변경이 있으면 업데이트
